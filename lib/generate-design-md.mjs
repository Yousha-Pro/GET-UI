export function generateDesignMarkdown(context) {
  ...
  const framework = normalized.siteSignals?.framework || "Unknown";
  ...
  const antiPatternSection = normalized.antiPatterns?.length
    ? `\n## Design Anti-Patterns\n${normalized.antiPatterns.map(p => `- ${p}`).join("\n")}\n`
    : "";

  const qaChecklist = generateDynamicQAChecklist(normalized);
...
## Design Rationale
${normalized.designRationale || "Standard design system implementation based on extracted site patterns."}
...
function generateDynamicQAChecklist(normalized) {
  const base = [
    "- [ ] All extracted tokens are mapped to semantic variables.",
    "- [ ] Color contrast meets WCAG 2.2 AA standards.",
    "- [ ] Typography hierarchy is consistent across element types."
  ];

  const components = normalized.componentHints || [];
  for (const comp of components) {
    if (comp.type === "buttons") {
      base.push("- [ ] Buttons define all 7 required states (hover, active, etc.).");
    }
    if (comp.type === "inputs") {
      base.push("- [ ] Form inputs have clear labels and visible focus rings.");
    }
    if (comp.type === "navigation") {
      base.push("- [ ] Navigation menus are keyboard accessible and ARIA labeled.");
    }
  }

  if (normalized.antiPatterns?.length) {
    base.push("- [ ] Identified Design Anti-Patterns have been reviewed and addressed.");
  }
...
function joinTokens(rows, limit) {
  ...
  return rows
    .slice(0, limit)
    .map((row) => {
      const varSuffix = row.variable ? ` (alias: \`${row.variable}\`)` : "";
      return `\`${row.token}=${row.value}\`${varSuffix}`;
    })
    .join(", ");
}
